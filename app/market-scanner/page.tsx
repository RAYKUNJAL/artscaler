    ...
    async function handleScrape() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setStatus('Scan started! Fetching real-time signals...');
                ...
                const pollInterval = setInterval(async () => {
                    ...
                    if (statusData.success) {
                        ...
                        if (job.status === 'completed') {
                            ...
                            setStatus('✓ Scan complete! Data points imported.');
                            setScraping(false);
                            loadData();
                        } else if (job.status === 'failed') {
                            clearInterval(pollInterval);
                            setStatus(`✗ Error: ${job.error_message || 'Scrape failed'}`);
                            setScraping(false);
                        } else {
                            setStatus(`Scanning market... ${job.items_found || 0} items identified so far.`);
                        }
                    }
                }, 3000);
            } else {
        ...
        <DashboardLayout>
            <div className="space-y-8">
                ...
                <div className="flex items-start justify-between">
                    <div>
                        <h1 className="text-3xl font-bold text-white mb-2">Market Intelligence Scanner</h1>
                                ...
                                {loading && listings.length === 0 ? (
                                    ...
                                    listings.map((item) => (
                                        <tr key={item.id} className="hover:bg-gray-800/30 transition-colors group">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col">
                                                    <span className={`text-lg font-black ${getWvsColor(item.demand_score || 0)}`}>
                                                        {item.demand_score ? item.demand_score.toFixed(1) : 'PENDING'}
                                                    </span>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <a
                                                    ...
                                                    className="p-2 bg-gray-800 hover:bg-blue-600 rounded-lg inline-block transition-all text-gray-400 hover:text-white"
